...
/**
 * 适用于微服务级别的限流，要想对每个微服务实现精准限流，需要在run方法里面，
 */
@Slf4j
public class ZuulRateLimitForEveryServerFilter extends ZuulFilter {
    ...
    public Object run() {
        try {
            ...
            if (serviceId != null) {
                key = serviceId;
                map.putIfAbsent(serviceId, RateLimiter.create(0.6));
            }
            // 如果压根不走RibbonRoutingFilter，则认为是URL格式的路由
                ...
                if (routeHost != null) {
                    url = routeHost.toString();
                }
                key = url;
                map.putIfAbsent(url, RateLimiter.create(0.8));
            }
            ...
            log.error("微服务过滤器正在限流：" + e.getMessage());
        }
        return null;
    }
